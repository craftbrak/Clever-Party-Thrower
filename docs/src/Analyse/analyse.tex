...
Il m'a alors fallu choisir aussi un \Gls{orm} pour interfacer avec ma base de données.
...
Prisma est une \Gls{orm} un peu particulière dans le sens où il propose d'utiliser un schema prisma pour définir ses entités au lieu d'utiliser du code TypeScript.
...
Sans compter que les opérations de \Gls{crud} dans les entités étaient rendues très complexe dû aux liens entre elles et que Prisma ne permet pas d'utiliser des identifiants.\\\\
J'ai donc décidé de jeter un oeil à TypeOrm, celui-ci propose de définir les entités via des annotations TypeScript.
Ce qui me permet d'utiliser une seule classe pour définir mon entité d'\Gls{orm} et de graphQl de plus TypeOrm est mieux
...
\paragraph{Sécurité}
L'application n'utilise pas \Gls{https} vu que cela nécessiterait des certificates SSL, cependant il est vivement conseiller d'utiliser un reverse proxy pour réaliser la terminaison SSL.\\
